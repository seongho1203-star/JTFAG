...
/* 독수리 연속 달성 축포에 쓸 **큰** 그림.
   비워 두면 앱이 직접 그린다(`eagleSvg()` in ui.js).
   주소를 넣으면 그걸 대신 띄운다 — GIF · APNG · WebP · PNG 아무거나 된다.

   투명 배경이 필요하면 GIF 말고 **APNG나 WebP**를 쓸 것. GIF는 투명도가
   켜짐/꺼짐 두 단계뿐이라 어두운 하늘 위에서 흰 테두리가 남는다.
   배경이 원래 어두운 GIF라면 그냥 써도 깔끔하다.
...
/* 골프장 안의 9홀 코스 이름.
   **`courses.js`에는 이게 없다** — OSM은 골프장 하나를 점 하나로만 갖고 있어
   코스 이름을 받아올 데가 없다. 그래서 우리가 가는 곳만 손으로 적어 둔 표다.
   여기 없는 골프장은 목록이 안 뜨고 직접 쳐 넣게 된다(쳐 넣은 이름은 다음부터 칩으로 뜬다).

   키는 `clubKey()`로 다듬은 이름이다 — 띄어쓰기와 끝의 `CC` · `컨트리클럽` 따위를 뗀 것.
   **딱 맞을 때만 쓴다.** `화순`과 `화순엘리체`는 다른 골프장이라 앞부분만 같다고 붙이면 틀린다.
   새 골프장을 넣을 땐 그 골프장 공식 코스 이름을 확인하고 적을 것 —
   **모르면 비워 두는 게 맞다.** 없으면 직접 쳐 넣으면 되지만, 틀린 이름이 목록에 뜨면
   그걸 눌러 저장하게 되어 기록이 잘못 남는다. */
const CLUB_COURSES = {
    "함평엘리체": ["임페리얼", "마제스티", "펠리스"],
    "어등산": ["어등", "송정", "하남"],
    "해피니스": ["휴먼", "해피", "하트", "힐링", "히든"],
...
// 같은 곳을 달리 적는 표기. 체인 이름이 앞에 붙으면 위 키와 안 맞아 따로 걸어 준다.
CLUB_COURSES["골프존카운티무등산"] = CLUB_COURSES["무등산"];
CLUB_COURSES["함평다이너스티"] = CLUB_COURSES["함평엘리체"];   // 옛 이름

// 골프장 이름을 표 키 모양으로 다듬는다. 띄어쓰기와 끝의 클럽 표기를 뗀다.
function clubKey(name) {
    return String(name || '').replace(/\s+/g, '')
        .replace(/(CC|GC|컨트리클럽|골프클럽|골프장|골프앤리조트|골프리조트|골프링크스|골프클럽스)$/i, '');
}

// 그 골프장의 9홀 코스 목록. 모르는 곳이면 빈 배열.
function clubCourses(name) {
    return CLUB_COURSES[clubKey(name)] || [];
}
