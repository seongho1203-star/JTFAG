...
function getGolferBadgesArray(g, overallMinAvg, overallMinScore) {
    ...
    if (badges.length === 0) badges.push({ html: `<div class="season-badge badge-rookie">🌱 루키</div>`, desc: "아직 획득한 뱃지가 없음" });
    return badges;
}
...
function calculateAndRender() {
    ...
    for (let r = appData.totalRounds - 1; r >= 1; r--) {
        ...
        golfers.forEach(g => {
            ...
            if (sCurr === "" || isNaN(parseFloat(sCurr))) completeCurr = false;
            if (sPrev === "" || isNaN(parseFloat(sPrev))) completePrev = false;
        });
        if (completeCurr && completePrev) { targetR1 = r - 1; targetR2 = r; break; }
    }
    ...
    document.querySelectorAll('#scoreTbody tr').forEach(row => {
        ...
        if (!isNaN(s1) && !isNaN(s2)) { if (avgCell) avgCell.textContent = `${Math.floor((s1 + s2) / 2)}`; } 
        else { if (avgCell) avgCell.textContent = `-`; }
    });
...
function processAllRoundSettlements() {
    ...
    golfers.forEach(g => {
        if (appData.scores && appData.scores[g]) {
            appData.scores[g].forEach(s => { const num = parseFloat(s); if (!isNaN(num) && num > 0 && num <= 79) golferSingleMap[g] = true; });
        }
    });

    // History and Rank Calculations
    const historyList = document.getElementById('historyList');
    if (historyList) historyList.innerHTML = "";
    document.querySelectorAll('.rank-cell').forEach(rc => rc.textContent = '-');

    for (let r = 2; r < appData.totalRounds; r++) {
        const handicapAvg = {}; const currentScores = {}; let isComplete = true;
        golfers.forEach(g => {
        ...
        for (let i = 0; i < golfers.length; i++) {
            for (let j = i + 1; j < golfers.length; j++) {
                ...
                const g1Adjusted = currentScores[g1] - (g1Avg - g2Avg);
                ...
                if (g1Adjusted < g2Adjusted) { matchResults[g1].wins++; matchResults[g2].losses++; } 
                    ...
                    if (g1Avg < g2Avg) { matchResults[g1].wins++; matchResults[g2].losses++; }
                    else if (g2Avg < g1Avg) { matchResults[g2].wins++; matchResults[g1].losses++; }
        ...
        sortedGolfers.forEach((golferName, index) => {
            ...
            roundHistoryHtml += `<div class="history-member"><span style="font-weight:700; color:var(--text-main);">${golferName}</span><div style="display:flex; align-items:center; gap:2px;"><span class="rank-badge ${rankInfo.class}">${rankInfo.icon} ${rankInfo.name}</span><span style="font-weight:800; color:${profit > 0 ? '#16a34a' : (profit < 0 ? '#dc2626' : '#64748b')};">${priceDisplay}</span></div></div>`;
            const row = document.querySelector(`tr[data-name="${golferName}"]`);
            if (row) { const rc = row.querySelector('.rank-cell'); if (rc) rc.innerHTML = `<span class="rank-badge ${rankInfo.class}">${rankInfo.icon} ${rankInfo.name}</span>`; }
        });
    ...
    golfers.forEach(g => {
        const validScores = (appData.scores && appData.scores[g]) ? appData.scores[g].filter(s => s !== "" && !isNaN(parseFloat(s))).map(s => parseFloat(s)) : [];
        if (validScores.length > 0) {
            const userMin = Math.min(...validScores);
            golferMinScores[g] = userMin;
            if (userMin < overallMinScore) overallMinScore = userMin;
            const userAvg = validScores.reduce((acc, curr) => acc + curr, 0) / validScores.length;
            golferAvgScores[g] = userAvg;
            if (userAvg < overallMinAvg) overallMinAvg = userAvg;
        } else { golferMinScores[g] = null; golferAvgScores[g] = null; }
    });
        ...
        golfers.forEach(g => {
            ...
            if (appData.roundMoney) {
                for (let rIdx = 0; rIdx < appData.totalRounds; rIdx++) {
                    const rMoney = appData.roundMoney[rIdx]; if (!rMoney) continue;
            ...
            let finalColor = finalBalance > 0 ? "#16a34a" : (finalBalance < 0 ? "#dc2626" : "#64748b");
            
            summaryGrid.innerHTML += `
                <div class="summary-item">
                    <div class="name" onclick="openPersonalReport('${g}')">${g}</div>
                    <div class="detail-line"><span class="label">계급</span> <span class="val">${rankProfit === 0 ? "0원" : (rankProfit > 0 ? "+" : "") + (rankProfit / 10000).toFixed(1) + "만"}</span></div>
                    <div class="detail-line"><span class="label">타수</span> <span class="val">${totalPureStrokeProfit === 0 ? "0원" : (totalPureStrokeProfit / 10000).toFixed(1) + "만"}</span></div>
                    ${summaryBadgesHtml}
                    <div class="final-total" style="color: ${finalColor};">합산: ${finalBalance === 0 ? "0원" : (finalBalance > 0 ? "+" : "") + (finalBalance / 10000).toFixed(1) + "만"}</div>
                </div>
            `;
        });
    }
