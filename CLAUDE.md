...
- **찬조는 모임을 위해 개인이 쓴 돈이다.** 정산에서 나온 값이 아니라 본인이 따로 적고,
  **합산에서 그만큼 뺀다** — 합산이 -50만인 사람이 5만을 찬조하면 -55만이 된다.
  ...
  - **표의 6번째 열로 넣지 말 것.** 5열이 이미 364px(390px 기기)를 꽉 쓰고 있어
    ...
    그래서 표 **아래에 따로** 두 칸씩 두 줄로 뒀다 — 입력칸이 127px로 정산 칸(68px)보다 넓다.
...
- **니어 이월은 입력받지 않고 계산한다** (`paintNearBox()` in ui.js).
  ...
  그래서 **그 차수의 잃은 돈과 딴 돈의 합이 안 맞는다** — 8차가 -94,000 / +53,000이었고
  남은 41,000원이 팟에 남아 넘어간 돈이다.
  - `잃은 + 딴`이 정확히 그 금액이라 **따로 적게 하지 말 것** — 적어 두면 시작·남은 금액을
    고칠 때마다 어긋난다. 계산하면 언제나 맞는다. payload에 새 필드도 안 생긴다.
  - 차액이 **플러스일 수도 있다** — 지난 차수에서 넘어온 이월을 이번에 누가 먹은 경우다.
    그때는 문구가 `이월 받음`으로 바뀌고 색도 달라진다(`carry-in`). 0이면 `없음`.
  - **찬조와 섞지 말 것.** 니어는 표의 `시작`·`남은`만 보고, 찬조는 `donate`만 본다.
    셋이 한 줄에 있어도 계산은 완전히 따로다.
  - 자리는 찬조 왼쪽 · 니어 오른쪽으로 반씩 나눠 쓴다(`.donate-row`가 flex).
    니어가 오른쪽 절반을 가져가므로 **찬조는 한 줄에 하나씩 쌓는다** —
    두 칸씩 늘어놓으면 절반 폭에서 입력칸이 40px로 찌그러진다.
