...
- **타수 표는 양끝이 고정돼 있고 가운데 차수만 움직인다.** 왼쪽이 이름(`.sticky-col-1`),
  오른쪽이 평균(`.sticky-col-right`)이다. 고정 칸에는 **불투명한 배경을 반드시 줄 것** —
  비치면 밑으로 지나가는 타수가 겹쳐 보인다. 평균 열의 `z-index`는 이름 열(3)보다 낮은 2다.
  화면이 아주 좁아 둘이 만나면 이름이 위로 오게 한 것이다.
