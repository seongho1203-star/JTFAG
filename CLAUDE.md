...
- **사진 보기(`#imageViewModal`)는 앱이 직접 확대한다** (`photoZoom` in ui.js).
  ...
  **`transform`만 움직인다**(다른 연출과 같은 이유 — 안드로이드에서 filter는 끊긴다).
  ...
  - 도구는 **다운로드 · 공유 · 닫기** 셋뿐이고 SVG 아이콘이다(그림문자 ⬇️·✕는 뺐다).
    **다운로드와 공유를 갈랐다** — 예전엔 다운로드가 공유 창을 먼저 띄웠는데 사용자가 둘을 따로
    원했다. 공유는 `navigator.share`에 파일을 넘긴다.
    파일 공유가 안 되면 주소라도 넘기고, 그것도 없으면 안내만 한다. 공유 창을 그냥 닫은
    `AbortError`는 오류가 아니다. 파일 이름의 확장자는 `PHOTO_EXT`로 실제 종류에서 딴다.
  - **폰에서 다운로드는 `<a download>`를 쓰지 않는다** (`downloadCurrentPhoto()`).
    ...
    손가락 기기(`isTouchDevice()` = `maxTouchPoints > 0`)에서는 다운로드도 그 시트를 띄운다.
    공유 버튼과 다른 건 제목(`JTFAG 사진 저장`)과 안내 토스트뿐이다. 시트를 그냥 닫은
    `AbortError`는 조용히 넘긴다. **PC만 `<a download>`로 바로 저장한다.**
