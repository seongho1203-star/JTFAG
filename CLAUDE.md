...
- **잘못 올라간 기록은 창 안의 `🗑️ N차 홀 기록 지우기`로 지운다.** 8차 카드가 9차로
  올라가 par·rel이 한 글자도 다르지 않게 들어간 적이 있는데, **지울 길이 없어 막다른 길이었다** —
  `stats.js`는 저장소 파일이라 앱이 직접 못 고치고, 홀 기록이 있는 차수는 타수 칸이 잠겨
  관리자 메뉴로도 못 고친다. 이 버튼을 빼지 말 것.
  - 앱은 `scoreRequests`에 **`action: '삭제'`** 요청만 남긴다(사진도 URL도 없다).
    판독과 **같은 줄**을 타므로 깨우기·상태·알림이 그대로 재사용된다 —
    `processOne()`이 맨 앞에서 갈래를 타고 `removeOne()`이 처리한다.
  - `removeRoundBlock()`은 **앞뒤 쉼표까지 본다.** 뒤에 형제가 있으면 그 쉼표를, 마지막
    블록이면 앞 쉼표를 먹는다 — 안 그러면 `}, }`가 남아 형식이 깨진다.
    쓴 뒤 `node --check`로 확인하고 깨졌으면 되돌린다(판독과 같은 안전장치).
  - 없는 차수를 지우라고 하면 파일을 안 건드리고 사유를 남긴다.
  - **지워도 `payload.scores`의 타수는 안 사라진다** — `syncScoresFromHoles()`가 홀 기록이
    있는 차수만 채우기 때문이다. 대신 칸이 다시 열리므로 올바른 사진을 올리거나
    관리자 메뉴 → `✏️ 타수 직접 수정`으로 고치면 된다. 확인창에 이 말을 적어 뒀다.
...
- **일정은 `payload.nextRoundNo`로 차수에 붙어 있다.** 그래서 차수를 지워도 일정은 지운 번호를
  붙들고 있었다 — 10차를 지웠는데 일정 창을 열면 여전히 `10차 (예정) · 골프장`이 골라져 있었다.
  이제 `removeRound()`가 `carryScheduleAfterRemove()`로, 접속 때는 `applyHoleScores()`가
  `settleScheduleRound()`로 같은 규칙을 적용한다: **일정이 붙은 차수가 지워졌거나 '아직 없는
  다음 차수'인데 남은 마지막 차수가 비어 있으면** 일정을 그 마지막 차수로 옮긴다
  (`moveScheduleTo()` — `nextRoundNo`·`roundCourses`·`roundSubCourses`·표의 골프장 칸을 함께
  옮기고 옛 번호의 것은 지운다).
  - **'비어 있다'는 `roundIsEmpty()`다** — 홀 기록·타수·정산 금액(찬조 포함)·사진이 **모두** 없을 때.
    홀 기록만 보면 안 된다 — 지난 차수를 치고 스코어카드를 아직 안 올린 사이에 다음 일정을
    잡은 경우, 그 차수를 안 친 것으로 잘못 봐서 골프장을 덮어쓴다. `payload.scores`만 보는 것도
    안 된다 — 홀 기록을 지운 뒤에도 타수가 남아 있을 수 있어서다. 하나라도 있으면 그대로 둔다.
  - 접속 때 정리는 바뀐 게 있을 때만 저장한다(헤드리스로 두 번째 호출에 저장 0회 확인).
- **9홀 코스는 골프장과 섞지 않는다.** 한 클럽에 코스가 여럿이고(함평엘리체 = 임페리얼 ·
  마제스티 · 펠리스) 한 라운드는 그중 둘을 돈다. 일정 모달의 `🏁 코스` 칸에 적으면
  **`payload.roundSubCourses`**(키가 차수 번호, `roundCourses`와 같은 모양)에 따로 들어간다.
  **`courses[]`에 합쳐 넣지 말 것** — `함평엘리체 (마제스티-펠리스)`가 통째로 골프장 이름이 되어
  좌표를 못 찾아 날씨가 사라지고, `roundsByCourse()`에서 같은 클럽이 코스마다 갈라져
  골프장별 승률이 조각난다. 선택 입력이라 비어 있는 게 정상이다.
  - 일정 문구(`nextRoundDate`)에는 `... 함평엘리체 (마제스티-펠리스)`처럼 괄호로 붙는다.
    날씨는 `courseFromText()`의 '문구 안에서 가장 긴 이름' 갈래가 잡아 준다.
    대신 **`lastScheduledCourse()`는 끝의 괄호를 떼고** 클럽 이름만 돌려준다 — 안 떼면
    다음 차수 일정을 열 때 괄호까지 검색칸에 들어앉아 그대로 골프장 이름이 되어 버린다.
  - **지난 차수의 코스는 아무도 입력하지 않아도 이미 있다.** 스코어카드 판독이 카드에 인쇄된
    코스 이름을 `ROUND_HOLES[n].course`(stats.js)에 적어 왔다 —
    `함평엘리체 (마제스티-펠리스)`처럼 골프장까지 있기도 하고 `어등-송정`처럼 코스만 있기도 하다.
    앱이 그걸 안 읽고 있었을 뿐이라, **골프장별 코스 정보를 어디서 받아 올 필요가 없다.**
    `subCourseFromStats()`가 괄호 안을 꺼내고, 괄호가 없으면 **하이픈으로 이어진 것만** 코스로 본다
    (골프장 이름 하나만 적힌 걸 코스로 오해하지 않게).
    `subCourseOf()`가 우선순위를 정한다 — **일정에서 정한 값이 먼저, 없으면 스코어카드 값**이다.
    **읽은 값을 payload에 옮겨 적지 말 것** — stats.js가 원본인데 두 곳이 되면 어긋나고,
    저장이 나가면 실시간 이벤트가 되돌아와 렌더가 또 돈다. 읽기만 한다.
    (OSM에는 코스 정보가 없다. `courses.js`로 받아 오려는 시도를 되풀이하지 말 것.)
  - **코스 목록은 `CLUB_COURSES`(api.js)에 손으로 적어 둔 표다.** `courses.js`에는 코스가 없다 —
    OSM은 골프장 하나를 점 하나로만 갖고 있어 받아올 데가 없다. 그래서 우리가 가는 곳만 적었다
    (함평엘리체 · 어등산 · 해피니스 · 무등산 · 골드레이크 · 화순 · 광주 · 보성 · 다산베아채 ·
    JNJ · 파인비치). **모르는 골프장은 비워 두는 게 맞다** — 없으면 직접 쳐 넣으면 되지만,
    틀린 이름이 목록에 뜨면 그걸 눌러 저장하게 되어 기록이 잘못 남는다.
    새로 넣을 땐 그 골프장 공식 코스 이름을 확인할 것.
  - 키는 `clubKey()`로 다듬은 이름이다(띄어쓰기와 끝의 `CC`·`컨트리클럽` 따위를 뗀 것).
    **딱 맞을 때만 쓴다** — `화순`과 `화순엘리체`는 다른 골프장이라 앞부분만 같다고 붙이면 틀린다.
    체인 이름이 앞에 붙는 표기(`골프존카운티무등산`)는 별칭으로 따로 걸어 둔다.
  - **코스는 눌러서 고르고, 누를 때마다 켜지고 꺼진다**(`toggleSubCourse()`). 한 라운드가
    두 코스를 도니까 `마제스티` → `펠리스` 순으로 누르면 `마제스티-펠리스`가 된다.
    **고른 차례가 곧 전·후반 순서라 정렬하지 말 것** — 칩의 `1`·`2` 숫자가 그 순서다.
    칸에 직접 쳐도 되고(`oninput`이 칩을 다시 켠다), 표에 없는 골프장은
    한 번 쳐 넣으면 `pastSubCourses()`가 기록에서 뽑아 다음부터 목록에 올린다.
  - **골프장을 바꾸면 골라 둔 코스를 비운다**(`pickCourse()`). 안 비우면 어등산인데
    마제스티가 붙은 채로 저장된다.
    사용자가 친 글이라 **칩은 `onclick` 문자열이 아니라 리스너로 만든다** — `escapeHtml`이 만든
    `&#39;`는 속성값에서 다시 `'`로 풀려 그 자리에서 코드가 깨진다.
  - 표 머리의 코스 줄은 `paintSubCourse()` 한 곳에서만 그린다.
    **`renderTable()`의 빠른 길(값만 갈아 끼우는 분기)에서도 부른다** — 빠뜨리면 일정에서
    코스를 고쳐도 표는 새로고침 전까지 옛 코스를 붙들고 있다(정산 칸 잠금이 `data-lock` 없이
    그랬던 것과 같은 자리다).
...
- **금액 칸은 저장됐다는 표시가 있어야 한다.** 확인 단추 없이 포커스가 빠질 때(`onchange`)
  저장되는데, 표시가 없으니 "다른 데를 눌렀는데 들어간 건지 모르겠다"는 말이 나왔다.
  아이폰 숫자 키패드에는 완료 키조차 없다(홈 화면 앱은 사파리의 '완료' 줄도 안 뜬다).
  그래서 셋을 뒀다 — 시작·남은·찬조 칸 모두 같다(`moneyCell()`·`renderDonateRow()`).
  - 저장되면 **`flashMoneySaved()`가 토스트로 `신성호 시작 금액 300,000원 저장됨`**을 띄우고
    그 칸을 초록으로 한 번 반짝인다(`.just-saved`). 배경색은 `.money-input`의 `!important`에
    막혀 애니메이션이 못 이기므로 **box-shadow**로 한다. 한 번만 돌고 끝난다.
    `updateMoney()`는 값이 같으면 저장을 안 내보내되 토스트는 띄운다.
  - 금액 칸에 커서가 있는 동안만 화면 아래에 **`✓ 입력 완료`**(`#moneyDoneBar`)가 뜬다
    (`focusin`/`focusout` 위임 · 잠긴 칸에서는 안 뜬다). 누르면 `finishMoneyInput()`이
    포커스를 빼서 저장을 태우고, 바뀐 게 없으면 `바뀐 금액이 없습니다`라고 한다.
    **단추의 `pointerdown`을 `preventDefault`로 막는다** — 안 막으면 누르는 순간 포커스가
    먼저 빠져 바가 사라지고 손가락이 엉뚱한 곳에 닿는다.
    아이폰은 키보드가 올라와도 `position:fixed`가 키보드 뒤에 깔리므로 `placeMoneyDoneBar()`가
    `visualViewport`를 보고 키보드 위로 올린다. 화면의 90%를 못 덮으니 `watchOverlays()`에는 안 걸린다.
  - Enter(안드로이드 키보드의 완료 키)를 누르면 `moneyKeydown()`이 포커스를 빼 저장된다.
- 순위별 벌금은 `RANK_CONFIG` (api.js)에 고정: 독수리 0 / 매 -40,000 / 학 -60,000 / 참새 -100,000원.
- **찬조는 모임을 위해 개인이 쓴 돈이다.** 정산에서 나온 값이 아니라 본인이 따로 적고,
  **합산에서 그만큼 뺀다** — 합산이 -50만인 사람이 5만을 찬조하면 -55만이 된다.
  요약 카드의 `계급`·`타수` 아래 `찬조` 줄에 보이고, 0이 아닐 때만 색이 붙는다.
  ...
  - 합계는 `donationOf()`(calc.js) 한 곳에서만 낸다. 요약 카드와 `golferFinalNetProfitMap`
    (기부왕 뱃지 = '합산 손실 1위')이 같이 쓰므로 두 군데가 되면 뱃지와 숫자가 어긋난다.
  - **타수정산에 섞지 말 것.** `pureStrokeDiff`는 `end - start`에서 계급분을 뺀 값이고
    찬조는 거기 들어가지 않는다 — 섞으면 차수별 표와 요약이 안 맞는다.
  - **표의 6번째 열로 넣지 말 것.** 5열이 이미 364px(390px 기기)를 꽉 쓰고 있어
    하나를 더 넣으면 금액 칸이 찌그러진다(320px에서는 지금도 가로로 넘친다).
    그래서 표 **아래에 가로 한 줄로** 넷을 늘어놓았다. **이름을 위, 칸을 아래**에 둬야
    한 칸 87px 안에 들어간다 — 옆에 붙이면 이름이 자리를 먹어 입력칸이 45px로 찌그러진다.
    `.money-input`을 재사용하지 않는다(68px로 묶여 있다). `.donate-input`이 따로 있다.
  - 잠금은 정산 금액과 같은 규칙이다(`canEditMoney()` · 본인 칸만 · 관리자 전체 열기도 먹는다).
    `renderDonateRow()`도 **차수·잠금이 바뀔 때만 다시 만들고** 아니면 값만 갈아 끼운다 —
    매번 새로 만들면 타이핑 중에 포커스가 날아간다.
  - 입구는 `updateMoney(name, 'donate', ...)`로 정산 금액과 같다. 따로 만들지 말 것.
- **니어 잔액은 입력받지 않고 계산한다** (`paintNearBox()` in ui.js).
  파3 홀마다 **1인당 5천원씩** 걷는다 — 넷이면 한 홀에 2만원이고, 18홀에 파3가 넷이니
  한 차수에 8만원이 모인다. 그 홀의 니어 임자가 없으면 2만원이 팟에 남아 다음으로 넘어간다.
  그래서 **그 차수의 잃은 돈과 딴 돈의 합이 안 맞는다** — 8차가 -93,000 / +53,000이었고
  남은 40,000원이 잔액이다. 찬조 줄 아래에 한 줄로 적는다.
  - 잔액은 2만원의 배수가 된다(임자 없는 홀마다 2만원씩이라). 그래서 `파3 2개`처럼
    몇 홀인지 꼬리표를 붙여 봤는데 **사용자 요청으로 뺐다 — 되살리지 말 것.**
  - **그 배수 여부가 곧 검산이다**(`paintNearCheck()`). 오른쪽 끝 뱃지에 `✅ 정산 일치` /
    `⚠️ 불일치 5,000원`으로 적고, 어긋난 금액은 가까운 배수까지의 거리로 낸다.
    배수가 아니면 누군가의 시작·남은 금액이 잘못 적힌 것이다.
    **`잃은 + 딴 + 잔액`을 검산으로 쓰지 말 것** — 잔액을 그 둘의 합으로 계산하므로 언제나
    0이 나온다. 늘 '일치'만 뜨는 표시는 아무것도 안 알려 준다.
    아무도 금액을 안 적은 차수는 '일치'라고 하면 거짓말이라 `입력 전`으로 둔다.
    한 홀 몫은 `nearPotPerHole()`(인원수 × 5천원) 한 곳에만 적혀 있다.
  - **잔액 하나만 적는다.** 잃은·딴 금액도 같이 적어 봤는데, 표의 `타수정산` 칸에
    사람마다 이미 다 나와 있어 되풀이일 뿐이었다. 표에 없는 건 잔액 하나뿐이다.
  - **더하는 값은 표의 `타수정산` 칸이다**(`strokeDiffOf()`). 시작·남은 차이를 그대로 쓰면
    안 된다 — 거기엔 계급정산이 섞여 있는데 그 돈은 공금으로 가지 선수들 사이에서 오간 게
    아니다. 실제로 같은 차수에서 타수정산 합이 +159,000인데 생차이 합은 -41,000이었다.
  - **`strokeDiffOf()`·`rankPenaltyOf()`를 표와 같이 쓴다.** 규칙이 두 군데가 되면
    표에 적힌 타수정산을 눈으로 더한 값과 잔액이 안 맞아 어디가 틀렸는지 알 수 없게 된다.
    예전엔 `renderMoneyTable()`의 빠른 길과 전체 렌더에 같은 식이 두 번 적혀 있었다 —
    되돌리지 말 것.
  - 타수정산의 합이 정확히 잔액이라 **따로 적게 하지 말 것** — 적어 두면 시작·남은 금액을
    고칠 때마다 어긋난다. 계산하면 언제나 맞고 payload에 새 필드도 안 생긴다.
  - **마이너스일 수도 있다** — 지난 차수에서 넘어온 잔액을 이번에 누가 먹어, 팟에서 나간 돈이
    걷은 돈보다 많은 경우다. 부호를 그대로 붙여 적고 색으로 구분한다(`carry-in`). 0이면 `없음`.
  - **찬조와 섞지 말 것.** 니어는 타수정산만 보고, 찬조는 `donate`만 본다.
    위아래로 붙어 있어도 계산은 완전히 따로다.
  - 320px에서 합계 문구가 제목을 밀어 `찬`/`조`가 갈라졌던 적이 있다.
    제목은 `.donate-title`로 묶어 안 쪼개지게 하고, 자리가 모자라면 합계가 아랫줄로 내려간다.
...
- **독수리 연속 달성은 모임에서 크게 축하하기로 한 기록이라 따로 다룬다.**
  ...
  - **뱃지는 연속이 이어지는 동안만 붙는다. '한 번이라도 이뤘다'는 이름 옆 ⭐로 따로 남긴다.**
    매를 한 번 하면 `eagleStreak()`이 0이 되어 3연속을 이룬 기록이 어디에도 안 남았다.
    `bestEagleStreak()`(calc.js)가 **지난 기록 전체에서 가장 긴 연속**을 세고,
    3 이상이면 `paintEagleCrowns()`(ui.js)가 요약 카드와 타수 표의 이름 옆에 별을 붙인다.
    누르면 `eagleHonorNotice()`가 `⭐ 이관교님 · 독수리 3연속 달성자 (최고 4연속)`을 띄운다 —
    없으면 아는 사람만 아는 표시가 된다. 요약 카드에서는 `stopPropagation`으로
    개인 리포트가 안 열리게 막는다.
    - **계급 기록에서 계산한다 — payload에 새 필드를 만들지 말 것.** 차수를 지웠다 되살려도
      저절로 맞고, 저장이 안 나가니 실시간 이벤트가 되돌아오는 문제도 없다.
    - **표와 요약 카드가 `paintEagleCrowns()` 하나를 같이 쓴다.** 양쪽에 빈
      `<span data-crown="이름">`을 심어 두고 한 곳에서 채운다 — 규칙이 두 군데가 되면
      한쪽에만 별이 붙는다. 부르는 곳은 `calculateAndRender()`(계급을 낸 직후)와
      **`renderTable()`의 두 갈래 모두**다(빠른 길을 빠뜨리면 새로 3연속이 되어도
      새로고침 전까지 표에 안 붙는다 — 정산 칸 잠금이 `data-lock` 없이 그랬던 자리다).
    - 표의 이름 칸은 44px에 글자가 31.2px라 옆으로 8.8px밖에 안 남는다. 별을 흘려 넣으면
      칸이 47px로 벌어져 표 가로폭이 늘어나므로 **띄워서 이름 바로 위 가운데**에 얹는다.
      **애니메이션을 걸지 말 것** — 이름은 표 네 줄 + 카드 네 개가 늘 떠 있어
      '항상 켜져 있는 그리기 비용'이 된다.
...
- **사진 보기(`#imageViewModal`)는 앱이 직접 확대한다** (`photoZoom` in ui.js).
  viewport가 `user-scalable=no`라 브라우저의 두 손가락 확대가 안 먹기 때문이다. 포인터 이벤트로
  두 손가락 벌리기 · 한 손가락 끌기 · 두 번 두드리기(1 ↔ 2.5배) · 마우스 휠을 처리하고,
  **`transform`만 움직인다**(다른 연출과 같은 이유 — 안드로이드에서 filter는 끊긴다).
  이 창은 검은 바탕이라 `backdrop-filter`를 끈다 — 원본 사진 위에서 blur은 비싸고 보이지도 않는다.
  - 벌릴 때 **손가락 가운데 점이 제자리에 남는다.** 가운데를 기준으로 그냥 키우면 보고 있던 곳이
    밖으로 밀려난다. 그 점의 원본 좌표를 잡아 두고 배율을 바꾼 뒤 같은 자리에 오도록 이동값을 푼다
    (`photoZoomTo`·pinch 분기). `photoZoomClamp()`가 사진이 화면보다 커진 만큼만 밀리게 붙잡는다.
  - 무대(`.photo-stage`)에 `touch-action: none` — 없으면 페이지가 같이 밀린다.
  - **손을 뗀 자리가 사진 밖이고 움직인 적이 없을 때만 닫는다**(`moved` 플래그). 끌다 놓았다고
    닫히면 안 된다. 사진 위를 두드리면 닫히지 않고 두 번 두드림만 본다.
  - `setPointerCapture`는 try/catch로 감싼다 — 이미 풀린 포인터면 예외를 던져 나머지 손가락
    처리가 끊긴다(헤드리스 합성 이벤트에서 실제로 그랬다).
  - 도구는 **다운로드 · 닫기** 둘뿐이고 SVG 아이콘이다(그림문자 ⬇️·✕는 뺐다).
    **공유 단추는 뺐다.** 한때 다운로드·공유·닫기 셋이었는데, 폰에서 다운로드가 아래처럼
    시스템 시트를 띄우게 되자 공유와 똑같은 화면이 떠서 사용자 요청으로 지웠다
    (`shareCurrentPhoto()`도 함께 지웠다). 되살리지 말 것 — 시트 안에 공유 대상이 다 있다.
    파일 이름의 확장자는 `PHOTO_EXT`로 실제 종류에서 딴다.
  - **폰에서 다운로드는 `<a download>`를 쓰지 않는다** (`downloadCurrentPhoto()`).
    웹앱은 갤러리에 직접 쓸 수 없고, `<a download>`는 삼성에선 확인창이 뜨고 아이폰에선
    '파일' 앱으로 가서 "이상한 게 뜬다"는 말이 나왔다. 갤러리로 가는 유일한 길이
    시스템 시트(`navigator.share`에 파일)라 — 아이폰은 `이미지 저장`, 안드로이드는 갤러리·포토 —
    손가락 기기(`isTouchDevice()` = `maxTouchPoints > 0`)에서는 그 시트를 띄운다.
    시트를 그냥 닫은 `AbortError`는 조용히 넘긴다. **PC만 `<a download>`로 바로 저장한다.**
    `'ontouchstart' in window`로 판단하지 말 것 — PC 크롬에서도 true가 나올 때가 있다.
    폰에서 `<a download>`로 되돌리지 말 것.
  - 열고 닫을 때 `photoZoomReset()`으로 배율을 1로 돌린다 — 안 그러면 다음 사진이 확대된 채 뜬다.
...
- **변경 이력은 없앴다. 남은 로그는 공금뿐이다** (`fundLogs` · 관리자 메뉴의 `📜 공금 수정 로그`).
  ...
  따로 넣지 않았다.
  **한 건은 세 줄로 그린다** — `잔액 변화` / `적립·사용 + 금액` / `내역`.
  예전엔 셋을 한 줄에 붙여 놨는데 금액이 여섯 자리가 되니 `원)`만 다음 줄로 떨어져 읽기 나빴다.
  줄을 나누면 자릿수가 늘어도 모양이 안 무너진다. 되돌리지 말 것.
  부호가 0이면 `변동 없음`으로 적는다(`직접` 모드로 같은 값을 다시 넣은 경우).
  `memo`(내역)는 선택 입력이라 비어 있을 수 있고,
  ...
  **로그 창은 세로 3단(머리말·목록·버튼)이고 가운데만 스크롤된다** — 기록이 쌓여도 `닫기`가
  ...
  줄마다 `✎`(내역 고치기) · `✕`(그 기록만), 아래에 `전체 삭제`가 있다. 지우는 둘은
  **공금 잔액을 건드리지 않고** 로그만 지우며, 셋 다 `saveState()`를 먼저 불러 되돌리기가 된다.
  - **`✎`는 내역(memo)만 고친다. 금액은 못 고친다** (`editFundLog()` · `showMemoPrompt()`).
    로그는 `before → after`가 사슬로 이어져 있어 지난 금액을 고치면 다음 기록의 `before`와
    어긋나 이력이 거짓말이 된다. **금액을 잘못 넣었으면 공금을 한 번 더 수정해 바로잡는 게 맞다** —
    금액 수정을 여기에 넣지 말 것.
  - 내역을 비워서 저장하면 `memo` 키를 아예 지운다 — 이 기능 이전 기록과 같은 모양이 된다.
  - 값이 그대로면 저장하지 않는다. 실시간 이벤트가 되돌아오는 구조라 헛저장을 만들지 않는다.
  - `showMemoPrompt()`는 로그 창(z-index 9999) 위에 떠야 해서 `10001`을 쓴다.
    `.modal-overlay` 클래스는 안 쓴다(닫히면 `visibility:hidden`이라 인라인으로는 안 보인다).
