...
- **타수는 사람이 입력하지 않는다.** 화면의 타수 칸은 전부 잠겨 있고(`readonly`),
  ...
  관리자 메뉴 → `✏️ 타수 직접 수정`을 켜면 **모든 차수**를 손으로 고칠 수 있고, 고친 내역은
  `changeLogs`에 `…타수(수동)`으로 남는다.
  손으로 고친 칸은 `appData.scoreOverrides`(`"이름|차수인덱스"` → true)에 표식이 남고,
  **표식이 있는 칸은 `syncScoresFromHoles()`가 건드리지 않는다.** 이 표식이 없으면 다음 접속 때
  판독 값으로 되돌아가므로, 자동 입력 로직을 고칠 때 이 예외를 없애지 말 것.
  판독 값과 같은 수로 되돌리면 표식은 저절로 지워진다. 관리자 메뉴의
  `↩️ 스코어카드 값으로 되돌리기`는 표식을 전부 지우고 판독 값으로 복구한다.
