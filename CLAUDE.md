...
- **니어 이월은 입력받지 않고 계산한다** (`paintNearBox()` in ui.js).
  ...
  - **`strokeDiffOf()`·`rankPenaltyOf()`를 표와 같이 쓴다.** 규칙이 두 군데가 되면
    표에 적힌 타수정산과 니어의 합이 안 맞아 어디가 틀렸는지 알 수 없게 된다.
    예전엔 `renderMoneyTable()`의 빠른 길과 전체 렌더에 같은 식이 두 번 적혀 있었다 —
    되돌리지 말 것.
  - `잃은 + 딴`이 정확히 이월 금액이라 **따로 적게 하지 말 것** — 적어 두면 시작·남은 금액을
  ...
  - **찬조와 섞지 말 것.** 니어는 `시작`·`남은`(에서 계급분을 뺀 값)만 보고,
    찬조는 `donate`만 본다. 위아래로 붙어 있어도 계산은 완전히 따로다.
