...
- **니어 이월은 입력받지 않고 계산한다** (`paintNearBox()` in ui.js).
  파3 홀마다 1인당 5천원씩 걷어 니어한 사람이 가져가는데, 니어가 없으면 다음으로 넘어간다.
  그래서 **그 차수의 잃은 돈과 딴 돈의 합이 안 맞는다** — 8차가 -93,000 / +53,000이었고
  남은 40,000원이 팟에 남아 넘어간 돈이다. 찬조 줄 아래에 한 줄로 적는다.
  - **이월 금액 하나만 적는다.** 잃은·딴 금액도 같이 적어 봤는데, 표의 `타수정산` 칸에
    사람마다 이미 다 나와 있어 되풀이일 뿐이었다. 표에 없는 건 이월 하나뿐이다.
  ...
  - **`strokeDiffOf()`·`rankPenaltyOf()`를 표와 같이 쓴다.** 규칙이 두 군데가 되면
    표에 적힌 타수정산을 눈으로 더한 값과 이월이 안 맞아 어디가 틀렸는지 알 수 없게 된다.
    예전엔 `renderMoneyTable()`의 빠른 길과 전체 렌더에 같은 식이 두 번 적혀 있었다 —
    되돌리지 말 것.
  - 타수정산의 합이 정확히 이월 금액이라 **따로 적게 하지 말 것** — 적어 두면 시작·남은 금액을
    고칠 때마다 어긋난다. 계산하면 언제나 맞고 payload에 새 필드도 안 생긴다.
  - 합이 **플러스일 수도 있다** — 지난 차수에서 넘어온 이월을 이번에 누가 먹은 경우다.
    그때는 문구가 `이월 받음`으로 바뀌고 색도 달라진다(`carry-in`). 0이면 `없음`.
